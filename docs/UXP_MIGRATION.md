...
4. Add a local CCX package build script and wire it into `scripts/release_smoke.py`
...
- [x] Replace `cep_node.require("child_process")` calls with UXP alternatives — the CEP panel now routes its only OS action (Open Log) through the server-owned `POST /system/open-path` fixed targets, and ships without `--enable-nodejs` / `--mixed-context`, so there is nothing Node-shaped left to port
