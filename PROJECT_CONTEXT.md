...
|---|---|---|
| API routes | **1,370** | `opencut/_generated/route_manifest.json` (F099) |
| Blueprints | **101** | same |
...
| MCP extended route tools | **1,312 opt-in** | `opencut/_generated/mcp_extended_tools.json` (F194) |
...
DaVinci Resolve ────┤   WebSocket :5680           ├─ routes/* (101 blueprints) ─► core/* (527 modules) ─► FFmpeg / Whisper / Demucs / Torch / ONNX
...
1. **Chat-conductor agent (F143)** — Descript Underlord and FireRed-OpenStoryline have proven sidebar-chat + timeline-diff + post-turn self-review is the converging UX. OpenCut has every building block (1,370 routes, MCP sidecar, LLM abstraction) but no conductor. Highest-leverage gap.
