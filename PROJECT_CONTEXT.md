...
|---|---|---|
| API routes | **1,365** | `opencut/_generated/route_manifest.json` (F099) |
| Blueprints | **101** | same |
| Core processing modules (`opencut/core/`) | **525** Python files | `ls opencut/core` |
| Route files (`opencut/routes/`) | **101** | `ls opencut/routes` |
| Tests | **143 files** (≥7,600 tests claimed) | `ls tests/` |
...
| MCP extended route tools | **1,307 opt-in** | `opencut/_generated/mcp_extended_tools.json` (F194) |
...
DaVinci Resolve ────┤   WebSocket :5680           ├─ routes/* (101 blueprints) ─► core/* (525 modules) ─► FFmpeg / Whisper / Demucs / Torch / ONNX
...
1. **Chat-conductor agent (F143)** — Descript Underlord and FireRed-OpenStoryline have proven sidebar-chat + timeline-diff + post-turn self-review is the converging UX. OpenCut has every building block (1,365 routes, MCP sidecar, LLM abstraction) but no conductor. Highest-leverage gap.
...
9. **OpenTimelineIO Marker schema is now the F105 review-bundle interchange anchor** (Pass 48, F225; extended in Passes 49-51 with F226/F227/F229). Review bundles include `markers.otio`, deterministic SVG drawing overlays, `annotations/index.json`, `review_threads.json`, `premiere_markers.csv`, and `review_markers.edl` while preserving `markers.json`; remaining collaboration gaps move to F231/F233/F234 and the Later-tier F228/F230 items.
