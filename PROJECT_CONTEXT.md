...
11. **The route-readiness and MCP surfaces are better but still not complete** (Pass 41): F191 now adds `opencut/_generated/feature_readiness.json` with **58** route-derived readiness records across **67** direct route/check bindings, `/system/feature-state` exposes **84** records, F192 raises legacy `/openapi.json` typed response schemas from **30** to **100** route entries, F195 raises the curated MCP tool surface from **27** to **39** tools, F208 pins `/openapi.json` route coverage + path-parameter validity, and F209 pins every MCP tool/special action route against the live Flask app. F194 (generated extended MCP) and F193 (schema introspection) still need to close the broader typed-contract gap.
