...
11. **The route-readiness and MCP surfaces are better but still not complete** (Pass 43): F191 now adds `opencut/_generated/feature_readiness.json` with **58** route-derived readiness records across **67** direct route/check bindings, `/system/feature-state` exposes **84** records, F192/F193 raise legacy `/openapi.json` typed response schemas from **30** to **110** dataclass-discovered route entries, F195 raises the curated MCP tool surface from **27** to **39** tools, F194 adds **1,319** opt-in generated extended MCP route tools with **100** response-schema annotations, F208 pins `/openapi.json` route coverage + path-parameter validity, and F209 pins every curated MCP tool/special action route against the live Flask app. F196 still needs to close the broader single-source-of-truth gap between registry, checks, and model cards.
