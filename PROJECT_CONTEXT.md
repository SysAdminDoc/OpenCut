...
**Last consolidated:** 2026-05-18 (seventy-five autonomous research/verification/implementation/wrap-up passes, with Passes 1-34 on 2026-05-17 — see `.ai/research/2026-05-17/`). Pass 3 verified the live state, walked `host/index.jsx`, drafted the F143-F145 agent-conductor RFC, and quantified the market-fit story. Pass 4 ran the full release-smoke gate, fixed release-gate lint drift, and prepared the local research + hardening commit. Pass 5 closed F261/F262/F270 with cross-platform launchers, the UXP sample-link fix, and the README positioning refresh. Pass 6 closed F264/F266 with machine-readable npm-advisory release-smoke output and the CEP-residual/drop-QE migration note. Pass 7 closed F199 with a generated `/api` alias manifest and corrected the earlier "233 alias pairs" claim to 15 true aliases + 218 canonical `/api` routes. Pass 8 closed F191/F197 with a generated route/check readiness manifest and moved the `NON_AI_CHECKS` allowlist into the registry. Pass 9 closed F195 by expanding the curated MCP tool surface from 27 to 39 tools and adding MCP route/dispatch/path-validation tests. Pass 10 closed the local F202 notarization tooling with a macOS signing/notarytool workflow and docs; live Apple acceptance still needs configured repository secrets. Pass 11 closed F204 by generating/archiving/uploading the CycloneDX SBOM from the Linux release job. Pass 12 closed F207 by writing bundled FFmpeg version data into both installer manifests; F205 stayed open because the full coverage measurement timed out locally. Pass 13 closed F208 by normalizing Flask path parameters in `/openapi.json`, making OpenAPI operation IDs unique, and adding the OpenAPI contract to release smoke. Pass 14 closed F209 by fixing the stale `opencut_chat_edit` MCP route and adding live Flask route consistency coverage for every MCP route and special action route. Pass 15 closed F218 by pinning the core blueprint registration order and adding the route-collision/import-order test file to release smoke. Pass 16 closed F219 by adding SBOM completeness coverage for declared Python dependencies, model-card components, and CycloneDX dependency graph entries. Pass 17 closed F236 by adding FCC-style user-overridable caption display setting tokens, preview routes, and burn-in integration. Pass 18 closed F237 by adding a source-backed loudness standards registry and correcting the stale BS.1770-5 premise. Pass 19 closed F240 by adding source-backed caption reading-speed profiles and correcting unsupported Netflix/FCC/YouTube numeric assumptions. Pass 20 closed F241 by adding the FFmpeg/libass HarfBuzz/FriBidi text-shaping gate and wiring it into release smoke plus CI. Pass 21 closed F243 by making SRT UTF-8/no-BOM by default with an opt-in legacy Windows BOM toggle. Pass 22 closed F244 by surfacing Whisper segment confidence, language confidence, and Hindi/Arabic/low-confidence human-review flags across transcript routes, exports, cache/state, and CLI output. Pass 23 reattempted F205 coverage measurement but was interrupted after 36m46s; the partial 52.12% coverage JSON is invalid for raising the CI floor, so F205 remains open. Pass 35 closed F121/F122/F127a/F130/F133/F135 by raising source installs to Python 3.11+ and tightening Pillow, flask-cors, OpenCV, onnxruntime, and WhisperX dependency floors. Pass 36 closed F180 by adding the Wave N-T / F-number governance ledger and a drift test. Pass 37 closed F213 by adding a CI-only Inno install/uninstall smoke script and workflow wiring. Pass 38 closed F215 by expanding the fuzz harness to 13 targets and hardening the C2PA/plugin/CUBE parser surfaces it exercises. Pass 39 closed F216 by terminating registered child processes from `_cancel_job()` and adding a concurrent cancellation race test to release smoke. Pass 40 closed F214 by adding the ML / compose / TTS performance benchmark contract and release-smoke registry gate. Pass 41 closed F192 by expanding legacy OpenAPI typed response schemas to 100 route entries. Pass 42 closed F198 by promoting the CEP↔UXP parity matrix into a code-owned 18-function catalogue and generated JSON artifact. Pass 43 closed F194 by adding the opt-in generated extended MCP route-tool catalogue. Pass 44 closed F201 by building the recommended WPF installer in Windows tag/manual CI. Pass 45 closed F203 by adding Windows Authenticode signing workflow/tooling and renewal-policy docs. Pass 46 closed F223 with a caption Unicode validation gate for RTL, mixed bidi, Indic, and no-space CJK export preservation. Pass 47 closed F242 with shared Unicode-aware CJK caption line breaking for export, overlays, and shot-aware wrapping. Pass 48 closed F225 by adding an OpenTimelineIO `markers.otio` sidecar to F105 review bundles while preserving the legacy `markers.json` payload. Pass 49 closed F226 by adding SVG drawing annotation overlays plus `annotations/index.json` to F105 review bundles. Pass 50 closed F227 by adding `review_threads.json` threaded comments and review completion status to F105 review bundles. Pass 51 closed F229 by adding Premiere marker CSV and marker-only EDL sidecars to F105 review bundles. Pass 52 closed F231 by adding HMAC-signed local-LAN review portal share links with Caddy and mDNS descriptors. Pass 53 closed F233 by adding per-project Atom review feeds and optional HMAC-signed outbound review webhooks. Pass 54 closed F234 by adding croc/rclone delivery transfer bundles. Pass 55 closed F238 by upgrading the photosensitive flash checker to BT.1702/WCAG flash-pair analysis with red-flash and safe-gap reporting. Pass 56 closed F239 by adding a Microsoft `ai-audio-descriptions` compatible review-draft route with scene descriptions, dialogue context, gap assignment, and IndexTTS2-ready TTS hints. Pass 57 closed F249 by adding Flatpak/AppImage Linux release packaging, `io.github.sysadmindoc.opencut` desktop metadata, release workflow upload wiring, and Flathub boundary docs. Pass 58 closed F250 by adding disabled-by-default Aptabase telemetry with local settings, env overrides, app-key masking, property scrubbing, and `/telemetry/aptabase/*` routes. Pass 59 advanced F252.1 by adding a dormant Bolt UXP WebView scaffold with host API wrappers, message-bridge files, least-privilege config, and release-smoke guardrails. Pass 60 advanced F252.2 by adding a UXP host-action dispatcher for the 14 direct-UXP `ocXxx` actions plus explicit CEP fallback responses. Pass 61 closed F254 by wiring `Sequence.createSubsequence(ignoreTrackTargeting?)` behind the export-range dispatcher. Pass 62 closed F255 by adding the `EncoderManager.launchEncoder` / `exportSequence` / `startBatchEncode` handoff. Pass 63 closed F256 by adding UXP Transcript API helpers for supported-language inventory, clip transcript-state checks, optional transcript JSON export, and WebView host bridge exposure. Pass 64 closed F257 by adding UXP Object Mask state helpers for active-sequence and project-level detection. Pass 65 closed F258 by adding a UXP ProjectConverter AAF export helper with optional AAFExportOptions. Pass 66 closed F260 by adding generated UXP migration risk dashboard artifacts and a Settings-tab dashboard. Pass 67 closed F267 by adding generated UXP Developer Tool smoke-harness artifacts plus a bundled panel runner for the 14 direct-UXP host actions. Pass 68 closed F263 by adding the full optional-extras pip-audit gate, current Python advisory allow-list, and resolving `[all]` dependency drift. Pass 69 closed F271 by surfacing model-card hardware and minimum-VRAM metadata through feature readiness and CEP panel feature controls. Pass 70 closed F272 by adding a built-in wedding cinematic reel agent skill package and read-only `/agent/skills` catalogue routes. Pass 71 closed F193 by replacing the OpenAPI response-schema hand-table with dataclass discovery and raising typed `/openapi.json` coverage to 110 endpoints plus 100 extended-MCP response-schema annotations. Pass 72 closed F196 by adding the registry/model-card/check contract and raising `/system/feature-state` to 100 records. Pass 73 closed F206 by splitting PR Fast from the Release Full workflow. Pass 74 closed F210 by adding Vitest coverage for CEP/UXP panel utility seams and wiring it into npm, release-smoke, PR Fast, and Release Full. Pass 75 closed F212 by adding WPF installer xUnit coverage, quiet install/uninstall mode, Windows-runner headless smoke wiring, and the tracked icon build fix.
...
|---|---|---|
| API routes | **1,376** | `opencut/_generated/route_manifest.json` (F099) |
| Blueprints | **101** | same |
...
| MCP extended route tools | **1,319 opt-in** | `opencut/_generated/mcp_extended_tools.json` (F194) |
...
1. **Chat-conductor agent (F143)** — Descript Underlord and FireRed-OpenStoryline have proven sidebar-chat + timeline-diff + post-turn self-review is the converging UX. OpenCut has every building block (1,376 routes, MCP sidecar, LLM abstraction) but no conductor. Highest-leverage gap.
...
9. **OpenTimelineIO Marker schema is now the F105 review-bundle interchange anchor** (Pass 48, F225; extended in Passes 49-54 with F226/F227/F229/F231/F233/F234). Review bundles include `markers.otio`, deterministic SVG drawing overlays, `annotations/index.json`, `review_threads.json`, `premiere_markers.csv`, and `review_markers.edl` while preserving `markers.json`; LAN share links now use HMAC-signed review portal URLs with Caddy/mDNS descriptors, review activity now emits Atom feed entries plus optional HMAC-signed webhooks, delivery transfer bundles now provide croc/rclone handoff plans, leaving the Later-tier F228/F230 items.
...
11. **The route-readiness and MCP surfaces are better but still not complete** (Pass 43, updated Pass 72): F191 now adds `opencut/_generated/feature_readiness.json` with **58** route-derived readiness records across **67** direct route/check bindings, F196 adds registry-primary model-card/check cross-validation, `/system/feature-state` exposes **100** records, F192/F193 raise legacy `/openapi.json` typed response schemas from **30** to **110** dataclass-discovered route entries, F195 raises the curated MCP tool surface from **27** to **39** tools, F194 adds **1,319** opt-in generated extended MCP route tools with **100** response-schema annotations, F208 pins `/openapi.json` route coverage + path-parameter validity, and F209 pins every curated MCP tool/special action route against the live Flask app. Remaining visibility work is route-level coverage, not model-card/check drift.
