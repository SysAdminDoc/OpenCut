...
| Tests | **136 files** (≥7,600 tests claimed) | `ls tests/` |
...
11. **The route-readiness and MCP surfaces are better but still not complete** (Pass 9): F191 now adds `opencut/_generated/feature_readiness.json` with **58** route-derived readiness records across **67** direct route/check bindings, `/system/feature-state` exposes **84** records, and F195 raises the curated MCP tool surface from **27** to **39** tools. F192/F194 (typed OpenAPI + generated extended MCP) and F209 (MCP/route consistency) still need to close the broader ~1,250-route typed-contract gap.
