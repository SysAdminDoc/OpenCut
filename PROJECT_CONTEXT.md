...
**Last consolidated:** 2026-06-07 (381 autonomous research/verification/implementation/wrap-up passes, with Passes 1-34 on 2026-05-17 -- see `.ai/research/2026-05-17/`). Pass 3 verified the live state, walked `host/index.jsx`, drafted the F143-F145 agent-conductor RFC, and quantified the market-fit story. Pass 4 ran the full release-smoke gate, fixed release-gate lint drift, and prepared the local research + hardening commit. Passes 5-75 are recorded in ROADMAP.md and the pass update notes below. Pass 76 closed F220-F222 by adding external RVC backend execution/fallback handling, natural-language color-intent grading on `/ai/auto-grade`, cut-point pacing analysis on `/ai/pacing-analysis`, and route/catalogue tests. Passes 77-264 are summarized in the roadmap/history ledgers; Passes 265-381 are recorded below.

**Pass 381 update (no standalone research file):**
- Broadened the UXP i18n static key extractor to scan locale-shaped JavaScript string literals, so object-map keys such as deliverable labels and Settings plural summary keys are covered by the locale guard.
- Verified 861 UXP JavaScript locale-key literals with zero missing entries in `locales/en.json`.

**Pass 380 update (no standalone research file):**
- Added a static Spanish locale placeholder-parity guard comparing `locales/es.json` against `locales/en.json` so runtime format tokens such as `{count}`, `{error}`, `{platform}`, and `{output}` cannot be dropped by translated strings.
- Verified the guard on the full current Spanish UXP catalogue: 1,381 English keys, 1,381 Spanish keys, and no placeholder mismatches.

**Pass 379 update (no standalone research file):**
- Expanded the Spanish UXP locale pack across all Video workspace keys, including color match, auto-zoom, multicam, B-roll, depth, emotion, upscale, scene detection, style transfer, Shorts, social upload, and runtime feedback states.
- Tightened the Spanish-pack guard so `locales/es.json` must match the full current UXP English catalogue with placeholder parity, and verified the Spanish Video render in the in-app Browser.

**Pass 378 update (no standalone research file):**
- Expanded the partial Spanish UXP locale pack across all Captions workspace keys, including transcription setup, model/language/style selectors, chapter generation, repeat detection, result review, and runtime feedback states.
- Added a Spanish-pack guard requiring every `uxp.captions.*` key while keeping Video keys as the explicit English fallback sample, and verified the Spanish Captions render in the in-app Browser.

**Pass 377 update (no standalone research file):**
- Expanded the partial Spanish UXP locale pack across all Timeline workspace keys, including sequence write-back, OTIO export, beat markers, batch export, batch rename, smart bins, SRT prep, and generated readiness summaries.
- Added a Spanish-pack guard requiring every `uxp.timeline.*` key while keeping Captions and Video keys as explicit English fallback samples, and verified the Spanish Timeline render in the in-app Browser.

**Pass 376 update (no standalone research file):**
- Expanded the partial Spanish UXP locale pack across all Search workspace keys, including library-index setup, semantic search, NLP command parsing, result cards, and index-clear/runtime feedback.
- Added a Spanish-pack guard requiring every `uxp.search.*` key alongside the existing Agent, Deliverables, Settings, workspace, FCC, Audio, and Cut coverage, and verified the Spanish Search render in the in-app Browser.
...
- Added a Spanish-pack guard requiring every `uxp.settings.*` key alongside the existing Cut, Audio, FCC, guide, workspace, runtime, and status coverage, and verified the completed Spanish Settings render in the in-app Browser.

**Pass 373 update (no standalone research file):**
...
- Added a Spanish-pack guard requiring all `uxp.guide.*`, `uxp.workspace.*`, `uxp.runtime.*`, and `uxp.status.*` keys alongside the existing Cut, Audio, and FCC coverage, and verified the generated guide/status render in the in-app Browser.

**Pass 372 update (no standalone research file):**
- Expanded the partial Spanish UXP locale pack across all FCC caption display-settings keys: token labels, compliance notice copy, preview controls, live-preview feedback, and schema/error statuses now have Spanish keys.
- Added a Spanish-pack guard requiring all `uxp.fcc.*` keys alongside the existing Cut and Audio coverage, and verified the Spanish FCC render in the in-app Browser.

**Pass 371 update (no standalone research file):**
- Expanded the partial Spanish UXP locale pack across the full Audio workspace: denoise, normalization, loudness-match, beat-marker controls, and Audio runtime feedback now have Spanish keys.
- Added a Spanish-pack guard requiring all `uxp.audio.*` keys alongside the existing Cut coverage, and verified the Spanish Audio render in the in-app Browser.
...
- Seeded `locales/es.json` for first-viewport chrome, tab/workspace labels, connection state, and shared runtime essentials, with static guards proving the Spanish pack stays partial and falls back to English for uncovered keys.

**Pass 367 update (no standalone research file):**
- Extended UXP i18n into remaining shared and Settings generated/runtime feedback: backend reconnect/cancel toasts, relative-time fallbacks, live-update listener counts and titles, engine option labels, migration-risk row/tag summaries, update-available toasts, the Timeline batch-export no-clip guard, and Depth Anything install feedback now use locale keys.
- Added guard coverage for the shared/Settings runtime-generated slice so direct hardcoded Settings labels/statuses, update/depth install toasts, and shared Settings-adjacent runtime strings cannot return.
...
- Fixed Agent tab handlers to unwrap the shared backend client response shape before reading plan/review/enhance/variant/sequence-index/MCP payloads, and added guard coverage for those runtime paths.

**Pass 365 update (no standalone research file):**
- Extended UXP i18n into Search and Deliverables runtime feedback: search result metadata, indexing/search/NLP state, sequence readiness, deliverable selection summaries, document generation, and package status/error strings now use UXP locale keys.
- Added guard coverage for the Search/Deliverables runtime slice so direct hardcoded dynamic labels, statuses, toasts, and errors cannot return.
...
- Added guard coverage for the Shorts runtime slice so direct hardcoded review-board and pipeline feedback strings cannot return.

**Pass 362 update (no standalone research file):**
- Extended UXP i18n into Video tab AI/effects runtime feedback: B-roll generation, multimodal diarization, social upload/auth, depth effects, emotion highlights, B-roll analysis, AI upscale, scene detection, and style-transfer progress, success, status, and error strings now use UXP locale keys.
- Added guard coverage for the Video AI/effects runtime slice so direct hardcoded feedback strings cannot return.

**Pass 361 update (no standalone research file):**
- Extended UXP i18n into Video tab core runtime feedback: Color Match, Auto Zoom, and Multicam progress, success, status, and error strings now use UXP locale keys.
- Added guard coverage for the Video runtime feedback slice so direct hardcoded Color Match, Auto Zoom, and Multicam strings cannot return.

**Pass 360 update (no standalone research file):**
- Extended UXP i18n into Audio tab runtime feedback: denoise, normalization, loudness-match, and beat-marker progress, success, status, and error strings now use UXP locale keys.
- Added guard coverage for the Audio runtime feedback slice so direct hardcoded audio progress/status/toast strings cannot return.
...
- Added guard coverage for the Cut runtime feedback slice so direct hardcoded silence/filler progress, status, and result strings cannot return.

**Pass 357 update (no standalone research file):**
- Extended the UXP i18n foundation into shared runtime feedback: clipboard fallbacks, external URL guidance, picker fallbacks, repeated no-clip warnings, chat prefixes/defaults, action-count toasts, and Premiere API availability now use UXP locale keys.
- Added a shared `showSelectClipWarning()` helper and guard coverage so the repeated no-clip and picker fallback toasts cannot return as direct hardcoded `showToast(...)` strings.

**Pass 356 update (no standalone research file):**
- Extended the UXP i18n foundation into the Settings tab static and generated shell: Engine Routing, Live Updates Bridge, Migration Risk, Keyboard, About labels, ARIA labels, status text, empty states, bridge/engine/migration runtime statuses, and generated formatter strings now use UXP locale keys.
- Raised `tests/test_uxp_i18n.py` to require at least 660 static i18n attributes, representative Settings-tab keys, and `formatI18n()` dynamic-key coverage, lifting the current UXP static count to 661.
...
- Raised `tests/test_uxp_i18n.py` to require at least 560 static i18n attributes plus representative Deliverables-tab keys, lifting the current UXP static count to 564.

**Pass 353 update (no standalone research file):**
- Extended the UXP i18n foundation into the Search tab static shell: library indexing, semantic search, suggested prompts, empty states, NLP command controls, placeholders, ARIA labels, status text, and action buttons now use UXP locale keys.
- Raised `tests/test_uxp_i18n.py` to require at least 510 static i18n attributes plus representative Search-tab keys, lifting the current UXP static count to 514.
...
- Raised `tests/test_uxp_i18n.py` to require at least 370 static i18n attributes plus representative Shorts/Social Video-tab keys, lifting the current UXP static count to 373.

**Pass 350 update (no standalone research file):**
- Extended the UXP i18n foundation deeper into the Video tab shell: Emotion Highlights, B-Roll Analysis, Chat Editor, AI Upscale, Scene Detection, and Style Transfer controls, placeholders, select options, ARIA labels, hints, and action buttons now use UXP locale keys.
- Raised `tests/test_uxp_i18n.py` to require at least 325 static i18n attributes plus representative Video-tab keys, lifting the current UXP static count to 329.

**Pass 349 update (no standalone research file):**
- Extended the UXP i18n foundation into the top/core Video tab shell through Depth Effects: clip input, Color Match, Auto Zoom, Multicam Switch, Multimodal Diarization, B-roll Generation, and Depth Effects controls, placeholders, select options, ARIA labels, hints, and action buttons now use UXP locale keys.
- Raised `tests/test_uxp_i18n.py` to require at least 280 static i18n attributes plus representative Video-tab keys, lifting the current UXP static count to 283.

**Pass 348 update (no standalone research file):**
- Extended the UXP i18n foundation into the Audio tab shell: clip input, denoise method controls, normalization controls, loudness-match fields, beat-marker inputs, placeholders, option labels, checkbox text, and action buttons now use UXP locale keys.
- Raised `tests/test_uxp_i18n.py` to require at least 220 static i18n attributes plus representative Audio-tab keys, lifting the current UXP static count to 223.

**Pass 347 update (no standalone research file):**
- Extended the UXP i18n foundation into the F236 caption display-settings card: field labels, compliance notice copy, FCC source link, preview controls, live preview labels, dynamic status feedback, and preview sample fallback now use UXP locale keys.
- Updated the compliance-date runtime replacement to target only the date span, preserving the FCC source link, and raised `tests/test_uxp_i18n.py` to require at least 190 static i18n attributes plus representative FCC card keys.
...
- Raised `tests/test_uxp_i18n.py` to require at least 90 static i18n attributes plus representative Cut-tab keys, keeping the UXP locale coverage guard in release smoke.
...
- The i18n drift gate reports 2,564 keys, 2,564 consumers, 16 JS metadata consumers, 0 dead keys, and 0 missing keys. `extension/com.opencut.uxp/main.js` was already dirty during this pass and is intentionally preserved for the next cycle.

**Pass 342 update (no standalone research file):**
- Closed the June 6 CEP empty-state component finding by routing `buildEmptyHintMarkup()` through the shared `oc-empty-state`/`oc-empty-state-inline` classes while preserving existing hint semantics, ARIA live behavior, and tone styling.
- Added a localized Favorites empty state instead of hiding the Favorites bar when no pins are available, and expanded static coverage so job history, batch files, workflow steps, footage search, and favorites remain on the shared empty-state helper. Browser validation loaded the CEP shell from a local static server and verified the footage-search empty-state DOM/classes on a nonblank page; runtime Favorites interaction remains covered statically because the raw CEP classic-script shell is not the intended normal-browser runtime. Panel Vitest, ESLint, and build verification covered the local frontend surface.
...
- Added focused coverage for CSRF audit records, path-traversal audit records, rate-limit denial records, remote auth denial records, `/system/audit-log` reads, request-ID preservation, and token redaction.

**Pass 336 update (no standalone research file):**
- Closed the June 6 expression-engine thread-churn finding by replacing per-eval worker-thread spawning in `evaluate_expression()` with inline trace-deadline evaluation that restores any prior trace hook. A 30-second `evaluate_timeline()` regression now patches `threading.Thread` and proves no raw worker threads are created; the local benchmark evaluated 900 frames with no errors and held `threading.enumerate()` at 2 before and after.
- Advanced E15 to batch 171 by localizing CEP progress/results/footer chrome, command palette shell, preview/audio preview modals, clip context menu, and first-run wizard copy, plus extending the scanner/runtime updater to cover `data-i18n-alt`. The drift gate reports 2,515 keys, 2,515 consumers, 16 JS metadata consumers, 0 dead keys, and 0 missing keys.
...
**Pass 318 update (no standalone research file):**
- Advanced E15 to batch 157 by wiring the Settings Project Templates and AI Models static shell through locale hooks for card descriptions, template labels/placeholders, custom-template ARIA, refresh title/label, model-list ARIA, model refresh hint, total-size label, and idle inventory status. The live i18n drift report now shows 2,324 keys, 2,279 consumers, 45 dead keys, and 0 missing keys.
...
**Pass 321 update (no standalone research file):**
- Advanced E15 to batch 159 by wiring the Settings Audio & Zoom Defaults and GPU Recommendation static shell through locale hooks for default loudness, zoom amount, zoom easing options, recommendation labels, and recommendation action buttons. The live i18n drift report now shows 2,345 keys, 2,300 consumers, 45 dead keys, and 0 missing keys.

**Pass 322 update (no standalone research file):**
- Advanced E15 to batch 160 by wiring the Settings keyboard shortcut reference and About labels through locale hooks. The pass reused existing shortcut keys, added About-link keys, and reduced dead locale keys from 45 to 42; the live i18n drift report now shows 2,349 keys, 2,307 consumers, 42 dead keys, and 0 missing keys.

**Pass 323 update (no standalone research file):**
- Advanced E15 to cleanup batch 161 by removing six unused locale keys for audio, captions, silence, and shortcuts after the static shell proved it uses replacement keys, then adding a locale-file duplicate-key guard. The live i18n drift report now shows 2,343 keys, 2,307 consumers, 36 dead keys, and 0 missing keys.

**Pass 324 update (no standalone research file):**
- Advanced E15 to batch 162 by wiring Settings Preferences and Whisper CPU-mode labels through existing locale keys, then removing nine unused generic form locale keys after the scanner confirmed there were no live consumers. The live i18n drift report now shows 2,334 keys, 2,313 consumers, 21 dead keys, and 0 missing keys.

**Pass 325 update (no standalone research file):**
- Advanced E15 to batch 163 by wiring audio enhancement, loudness target, shorts export, and timeline marker export labels through existing locale keys. The live i18n drift report now shows 2,334 keys, 2,320 consumers, 14 dead keys, and 0 missing keys.

**Pass 326 update (no standalone research file):**
- Advanced E15 to batch 164 by removing the final 14 unused CEP locale keys after the scanner confirmed they had no live static consumers, then tightening the dead-key baseline to zero. The live i18n drift report now shows 2,320 keys, 2,320 consumers, 0 dead keys, and 0 missing keys.

**Pass 327 update (no standalone research file):**
- Advanced E15 to batch 165 by expanding `scripts/i18n_lint.py` to count supported JS locale-key metadata fields such as `labelKey`, adding regression coverage for supported and ignored key-field shapes, and keeping the zero-dead-key baseline intact. The live i18n drift report now shows 2,320 keys, 2,320 consumers, 16 JS metadata consumers, 0 dead keys, and 0 missing keys.

**Pass 328 update (no standalone research file):**
- Advanced E15 to batch 166 by localizing Auto Shorts form labels/options/buttons, Magic Clips review-board status/detail copy, the approved-render alert, and the Settings studio-readiness overview shell. The live i18n drift report now shows 2,360 keys, 2,360 consumers, 16 JS metadata consumers, 0 dead keys, and 0 missing keys.

**Pass 329 update (no standalone research file):**
- Advanced E15 to batch 167 by localizing the remaining CEP tab panel `aria-label` region names plus the Audio Normalize platform preset, loudness meter, and preview controls. The live i18n drift report now shows 2,372 keys, 2,372 consumers, 16 JS metadata consumers, 0 dead keys, and 0 missing keys.
...
**Pass 332 update (no standalone research file):**
- Closed the CLIP cache safe-deserialization item by replacing raw pickle embedding caches with compressed `.npz` files that store JSON metadata and load arrays with `allow_pickle=False`. Regression coverage verifies both the safe-load option and a no-pickle cache round trip.
...
**Pass 335 update (no standalone research file):**
- Closed the Gaussian splat preview confinement item by validating renderer-returned frame paths before `send_file()` and allowing only existing files under system temp or `~/.opencut`; regression coverage proves temp previews still serve and unconfined paths return 403. Advanced E15 to batch 170 by localizing Settings Operation Journal and Whisper readiness/default-model shell copy. The live i18n drift report now shows 2,457 keys, 2,457 consumers, 16 JS metadata consumers, 0 dead keys, and 0 missing keys.
...
|---|---|---|
| API routes | **1,539** | `opencut/_generated/route_manifest.json` (F099) |
| Blueprints | **107** | same |
