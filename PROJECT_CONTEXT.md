...
|---|---|---|
| API routes | **1,374** | `opencut/_generated/route_manifest.json` (F099) |
| Blueprints | **101** | same |
| Core processing modules (`opencut/core/`) | **536** Python files | `ls opencut/core` |
| Route files (`opencut/routes/`) | **101** | `ls opencut/routes` |
| Tests | **183 test_*.py files** (≥7,600 tests claimed) | `ls tests/` |
...
| MCP extended route tools | **1,317 opt-in** | `opencut/_generated/mcp_extended_tools.json` (F194) |
...
DaVinci Resolve ────┤   WebSocket :5680           ├─ routes/* (101 blueprints) ─► core/* (536 modules) ─► FFmpeg / Whisper / Demucs / Torch / ONNX
...
1. **Chat-conductor agent (F143)** — Descript Underlord and FireRed-OpenStoryline have proven sidebar-chat + timeline-diff + post-turn self-review is the converging UX. OpenCut has every building block (1,374 routes, MCP sidecar, LLM abstraction) but no conductor. Highest-leverage gap.
...
11. **The route-readiness and MCP surfaces are better but still not complete** (Pass 43): F191 now adds `opencut/_generated/feature_readiness.json` with **58** route-derived readiness records across **67** direct route/check bindings, `/system/feature-state` exposes **84** records, F192 raises legacy `/openapi.json` typed response schemas from **30** to **100** route entries, F195 raises the curated MCP tool surface from **27** to **39** tools, F194 adds **1,317** opt-in generated extended MCP route tools, F208 pins `/openapi.json` route coverage + path-parameter validity, and F209 pins every curated MCP tool/special action route against the live Flask app. F193 (schema introspection) still needs to close the broader typed-contract gap.
...
- F192 OpenAPI response-schema expansion lives in `opencut/schemas.py`, `opencut/openapi.py`, and `tests/test_openapi_contract.py`; `ROUTE_READINESS_AUDIT.md` records the new 100-entry typed schema surface
