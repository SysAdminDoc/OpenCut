...
|---|---|---|
| API routes | **1,362** | `opencut/_generated/route_manifest.json` (F099) |
| Blueprints | **101** | same |
...
1. **Chat-conductor agent (F143)** — Descript Underlord and FireRed-OpenStoryline have proven sidebar-chat + timeline-diff + post-turn self-review is the converging UX. OpenCut has every building block (1,362 routes, MCP sidecar, LLM abstraction) but no conductor. Highest-leverage gap.
...
- F223 caption Unicode validation lives in `opencut/core/caption_unicode_validation.py`, `opencut/tools/caption_unicode_validation.py`, `tests/test_caption_unicode_validation.py`, and the release-smoke `caption-unicode` step. It validates RTL, mixed bidi, Indic, Japanese, and Chinese fixtures through SRT, ASS, and burn-in ASS text export. F242 remains the explicit follow-up for real no-space CJK line breaking.
