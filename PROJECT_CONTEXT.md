...
- Pass 238 closed RA-34 by adding `requirements-lock.txt` as a default `opencut.tools.pip_audit_extras` target, carrying that target through release smoke, refreshing the lockfile `idna` pin to 3.16, and adding `--no-extras` for committed-requirements diagnostics while the separate RA-15 optional-extra advisories remain fail-closed.
