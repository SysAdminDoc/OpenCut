...
|---|---|---|
| API routes | **1,371** | `opencut/_generated/route_manifest.json` (F099) |
| Blueprints | **101** | same |
| Core processing modules (`opencut/core/`) | **528** Python files | `ls opencut/core` |
| Route files (`opencut/routes/`) | **101** | `ls opencut/routes` |
| Tests | **148 files** (≥7,600 tests claimed) | `ls tests/` |
...
| MCP extended route tools | **1,313 opt-in** | `opencut/_generated/mcp_extended_tools.json` (F194) |
...
DaVinci Resolve ────┤   WebSocket :5680           ├─ routes/* (101 blueprints) ─► core/* (528 modules) ─► FFmpeg / Whisper / Demucs / Torch / ONNX
...
1. **Chat-conductor agent (F143)** — Descript Underlord and FireRed-OpenStoryline have proven sidebar-chat + timeline-diff + post-turn self-review is the converging UX. OpenCut has every building block (1,371 routes, MCP sidecar, LLM abstraction) but no conductor. Highest-leverage gap.
