...
|---|---|---|
| API routes | **1,519** | `opencut/_generated/route_manifest.json` (F099) |
| Blueprints | **107** | same |
| Core processing modules (`opencut/core/`) | **598** Python files | `ls opencut/core` |
| Route files (`opencut/routes/`) | **105** (excluding `__init__.py`) | `ls opencut/routes` |
| Tests | **214 test_*.py files** + **2 Vitest panel test files** (8,400+ tests claimed) | `ls tests/`, `extension/com.opencut.panel/tests/` |
...
| `/api/*` routes | **233** total; **16** true aliases; **217** canonical `/api` routes | `opencut/_generated/api_aliases.json` (F199) |
...
| MCP extended route tools | **1,463 opt-in** | `opencut/_generated/mcp_extended_tools.json` (F194) |
...
DaVinci Resolve ────┤   WebSocket :5680           ├─ routes/* (107 blueprints) ─► core/* (598 modules) ─► FFmpeg / Whisper / Demucs / Torch / ONNX
...
1. **Chat-conductor agent (F143)** — Descript Underlord and FireRed-OpenStoryline have proven sidebar-chat + timeline-diff + post-turn self-review is the converging UX. OpenCut has every building block (1,519 routes, MCP sidecar, LLM abstraction) but no conductor. Highest-leverage gap.
...
11. **The route-readiness and MCP surfaces are better but still not complete** (Pass 43, updated Pass 87): F191 now adds `opencut/_generated/feature_readiness.json` with **66** route-derived readiness records across **90** direct route/check bindings, F196 adds registry-primary model-card/check cross-validation, `/system/feature-state` exposes **108** records, F192/F193 raise legacy `/openapi.json` typed response schemas from **30** to **110** dataclass-discovered route entries, F195 raises the curated MCP tool surface from **27** to **39** tools, F194 adds **1,463** opt-in generated extended MCP route tools with **99** response-schema annotations, F208 pins `/openapi.json` route coverage + path-parameter validity, and F209 pins every curated MCP tool/special action route against the live Flask app. Remaining visibility work is route-level coverage, not model-card/check drift.
