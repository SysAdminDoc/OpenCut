...
The latest published Windows installer is [OpenCut v1.55.1](https://github.com/SysAdminDoc/OpenCut/releases/tag/v1.55.1). It was built from the current source tree. Release artifacts are named `OpenCut-Setup-<version>.exe` and handle everything: server, FFmpeg, CEP extension, registry, and optional model downloads. No Python is needed for the packaged release.
...
| Bulk Transcript Correction | Preview and apply literal find/replace edits across the transcript, undo the last pass, and persist project glossary terms | Built-in |
...
fallback reason. API and CLI callers can override `engine`, `model`,
`model_revision`, and `language`; the panel auditions low-confidence edit
boundaries before it permits a filler-removal timeline mutation. A privacy-safe
`GET /captions/cache/provenance/<cache_key>` diagnostic returns model identity
without returning transcript text.
...
| Studio Graphite UI | Compact editorial workspace across dark, light, host-matched, and forced-colours modes |
| Workspace Polish | Flat sections, readable type, compact controls, and restrained status treatment across CEP and UXP |
