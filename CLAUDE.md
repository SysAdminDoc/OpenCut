...
- **All boolean flags must use `safe_bool()`** — Every `data.get("flag", True/False)` that feeds an `if` or downstream boolean parameter MUST wrap in `safe_bool(data.get("flag", default), default)`. Raw `bool(data.get(...))` coerces `"false"` to `True`. `safe_bool` handles `"true"/"false"/"yes"/"no"/"on"/"off"`, native bools, 0/1 ints, bytes/bytearray, and rejects NaN/inf/containers. All 26 flag sites across 8 route files were audited and fixed in v1.9.22.
- **`/file` preview route allowlist** — GET `/file` only serves files whose `realpath` is under `tempfile.gettempdir()` or `~/.opencut`. The MIME-type check (`audio/*`/`video/*`/`image/*`) is a second layer. Never remove the allowlist — it prevents same-browser attackers from enumerating local media via `<img>`/`<audio>` side-channels.
- **`startBackgroundPollers()` on reconnect** — `cleanupTimers()` nukes `mediaScanTimer` and `_statusTimer` on every disconnect. The reconnect branch in `checkHealth()` must call `startBackgroundPollers()` to restart them. Without this, media scanning and status bar polling silently stop after the first disconnect/reconnect until full reload.
- **`make_install_route` rate limit pattern** — The outer Flask handler acquires the rate-limit slot and returns 429 synchronously. The inner `@async_job` body runs in a worker thread and releases the slot in its `finally`. If the outer handler catches an exception before the thread starts, it releases the slot itself. This two-tier pattern avoids both double-release and slot-leak scenarios.
- **`/system/dependencies` 60s cache** — Cold call takes ~5s (20+ import_module + subprocesses). The result is cached in `_deps_cache` with a 60s TTL and thread-safe lock. `?fresh=1` query param bypasses the cache for post-install re-checks.
...
- **MCP docstring** fixed (claimed --sse existed). Added `resources/list` + `prompts/list` handlers.

## v1.9.13 Batch 45 (Complete ffmpeg Path Resolution)
- **23 bare `"ffmpeg"` in subprocess calls** — 13 in routes + 10 in core modules. ALL replaced with `get_ffmpeg_path()`. Bundled installs now work for every FFmpeg operation.
- Combined with batch 37's 14 ffprobe fixes: **every** subprocess call across the entire codebase now resolves FFmpeg/ffprobe via the cached path helper.

## v1.9.12 Batch 44 (Dependencies & Ports)
- **psutil** added to core deps (was optional but used by /system/status).
- **numpy** upper bound `<2` removed (dist already ships 2.x).
- **3 new optional dep groups** — otio, tts, depth.
- **social_post hardcoded port** — OAuth redirect URIs now use configurable port param.
...
- CLI now has 16 commands (up from 14).
