...
def test_apply_validates_and_clamps_ffmpeg_options(client, plugin_module, tmp_path):
    video = tmp_path / "clip.mp4"
    video.write_bytes(b"fake")
    ...
    with patch.object(
        plugin_module,
        "get_video_info",
        return_value={"fps": 29.97},
    ) as probe, patch("opencut.helpers.run_ffmpeg") as run_ffmpeg:
        resp = client.post(
            "/plugins/timecode-watermark/apply",
            json={
                "filepath": str(video),
    ...
    assert "rate=29.97" in filter_arg
    assert "x=w-tw-10:y=h-th-10" in filter_arg


def test_apply_rejects_frame_outside_probed_source_rate(client, plugin_module, tmp_path):
    video = tmp_path / "clip.mp4"
    video.write_bytes(b"fake")
    ...
    with patch.object(
        ...
        "get_video_info",
        return_value={"fps": 29.97},
    ), patch("opencut.helpers.run_ffmpeg") as run_ffmpeg:
        resp = client.post(
            "/plugins/timecode-watermark/apply",
            json={"filepath": str(video), "start_timecode": "00:00:01:30"},
        )
