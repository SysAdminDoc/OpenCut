...
- **Version everything.** `scripts/sync_version.py` keeps 19 targets in sync — run `python scripts/sync_version.py --check` before release work.
...
8. Tag with `git tag -a vX.Y.Z -m "OpenCut vX.Y.Z"`, then attach locally built artifacts to the release with `gh release create` / `gh release upload`.
