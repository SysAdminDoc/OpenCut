...
CARDS: List[ModelCard] = [
    ...
    ModelCard(
        check_name="check_diarization_available",
        feature_id="captions.pyannote-diarization",
        label="pyannote speaker diarization",
        category="captions",
        license="CC-BY-4.0 (community-1; legacy model terms vary)",
        upstream="https://huggingface.co/pyannote/speaker-diarization-community-1",
        hardware="cpu/gpu",
    ...
    ModelCard(
        check_name="check_nemo_asr_available",
        feature_id="captions.nemo-asr",
        label="NVIDIA NeMo ASR (Parakeet / Canary)",
        category="captions",
    ...
    ModelCard(
        check_name="check_sortformer_available",
        ...
        advisory_notes=[
            "Sortformer is an optional NeMo-gated diarization engine; verify the selected checkpoint license before redistribution.",
        ],
    ),
    # ---- Video --------------------------------------------------------
    ModelCard(
        check_name="check_autoshot_available",
        feature_id="video.scenes.autoshot",
        label="AutoShot gradual-transition scene detector",
        category="video",
        license="Custom (verify upstream before redistributing weights)",
        upstream="https://github.com/wentaozhu/AutoShot",
        hardware="cpu/gpu",
    ...
    ModelCard(
        check_name="check_depth_anything_3_available",
    ...
    ModelCard(
        check_name="check_iclight_available",
        feature_id="video.relight.iclight",
        label="IC-Light v1 relight",
        category="video",
        license="Apache-2.0",
        upstream="https://github.com/lllyasviel/IC-Light",
        hardware="gpu (>= 4 GB VRAM)",
        install_hint="pip install diffusers>=0.32 torch",
        privacy="local-only (model downloads only)",
        latency="slow",
        advisory_notes=[
            "OpenCut targets IC-Light v1 only; IC-Light v2 is not registered because the local module documents non-commercial/release concerns.",
        ],
    ),
    ModelCard(
        check_name="check_latentsync_available",
        feature_id="lipsync.latentsync",
        label="LatentSync diffusion lip-sync",
        category="lipsync",
        license="Apache-2.0 (code; checkpoint license unconfirmed)",
        ...
        latency="slow",
        advisory_notes=[
            "The engine is opt-in and not auto-selected until the checkpoint license is confirmed.",
        ],
    ),
    ModelCard(
    ...
    ModelCard(
        check_name="check_sam3_available",
        feature_id="video.sam3",
        label="SAM 3 text-prompted video segmentation",
        category="video",
        license="Custom (SAM License, commercial-permissive)",
    ...
    ModelCard(
        ...
        category="video",
        license="Apache-2.0",
        upstream="https://huggingface.co/ByteDance-Seed/SeedVR2-3B",
        hardware="gpu (>= 8 GB VRAM)",
        install_hint="pip install diffusers torch  # download seedvr2-3b via model manager",
        privacy="local-only (model downloads only)",
        latency="medium",
        quality_notes="Preferred upscaling engine when installed; falls back to Real-ESRGAN.",
    ),
    ModelCard(
    ...
    ModelCard(
        check_name="check_visual_multicam_available",
        ...
        privacy="local-only",
        latency="medium",
        quality_notes="Adds lip-movement and shot-size cues to audio-only multicam switching.",
    ),
    ModelCard(
    ...
    # ---- Vision niche --------------------------------------------------
    ModelCard(
        check_name="check_ocr_available",
        ...
        hardware="cpu/gpu",
        install_hint="pip install pytesseract easyocr  # plus system Tesseract for pytesseract",
        privacy="local-only",
        latency="medium",
        quality_notes="Used by multimodal footage search, OCR extraction, and OCR-based PII redaction.",
    ),
    ModelCard(
