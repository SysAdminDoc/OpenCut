...
| Release trust | Route manifest, OpenAPI checks, version sync, release smoke, pip/npm advisory gates, SBOM, installer policy, signing/notarization wiring, Linux packaging, generated README badges and non-badge count gates, Adobe tracker hardening for release-channel tags, deterministic probe exit codes, tracker labels, no-`gh` label dry-runs, Docker distribution-doc guards, and test-environment bootstrap checks. |
