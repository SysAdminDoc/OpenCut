...
| Release trust | Route manifest, OpenAPI checks, version sync, release smoke, pip/npm advisory gates, declared-only SBOM metadata/naming, installer policy, signing/notarization wiring, Linux packaging, generated README badges and non-badge count gates, Adobe tracker hardening for release-channel tags, deterministic probe exit codes, tracker labels, no-`gh` label dry-runs, Docker distribution-doc guards, and test-environment bootstrap checks. |
...
| Performance and recovery | The May 26 plan now has N1 closed: transcripts are cached by source SHA-256 plus backend/settings, with shared core integration and cache stats/clear routes; render-cache cleanup and invalidation also reject forged index output paths before unlinking. |
