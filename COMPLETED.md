...
| Area | Completed work |
|---|---|
| Core automation | Silence/filler removal, captions/transcription, caption round-trip sidecars, diff/apply revisions, CEP/hybrid caption write contracts, caption metadata-loss regression fixtures, sequence-index host locators, Magic Clips dry-run plan graph, approved-candidate render handoff, explainable scoring, and preset-driven multi-platform rendering, audio cleanup, video effects, export, timeline actions, deliverables, search, workflow presets, NLP commands, and plugin loading. |
