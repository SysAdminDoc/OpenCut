...
- Verification: `py -3.12 -m pytest -q tests\test_uxp_i18n.py` (18 passed); `py -3.12 -m pytest -q tests\test_uxp_i18n.py tests\test_roadmap_mirror.py tests\test_roadmap_lint.py` (33 passed); `py -3.12 -m ruff check tests\test_uxp_i18n.py`; JSON parse/count check verified 187 Spanish keys and representative Settings translations; in-app Browser QA at `http://127.0.0.1:8791/index.html?lang=es` verified backend-offline guide copy (`Backend desconectado`, `Actualizar backend`), Settings tab activation with `aria-selected="true"`, Spanish Settings section titles (`Enrutamiento de motores`, `Puente de actualizaciones en vivo`, `Riesgo de migracion`), Spanish engine/bridge/migration status text, hidden processing banner, and no unexpected console errors beyond the expected static-browser Premiere module warning; `py -3.12 scripts\sync_badges.py --check`; `py -3.12 scripts\check_doc_sizes.py --check`; `git diff --check`; `py -3.12 scripts\release_smoke.py --only ruff --json`; `py -3.12 scripts\release_smoke.py --only pytest-fast --json` (105 gate test files / 871 tests passed).
