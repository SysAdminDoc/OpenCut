...
- Verification: `py -3.12 -m pytest -q tests\test_uxp_i18n.py tests\test_uxp_caption_display_settings_ui.py` (19 passed, 33 subtests passed); `py -3.12 -m ruff check tests\test_uxp_i18n.py tests\test_uxp_caption_display_settings_ui.py`; Browser localhost check confirmed localized FCC card labels/status text, `lang=en`, and preserved eCFR link; direct `/captions/display-settings/tokens` backend check returned schema defaults; static UXP i18n count verified at 194 attributes; `py -3.12 scripts\release_smoke.py --only pytest-fast --json` (105 gate test files / 858 tests passed); `py -3.12 scripts\release_smoke.py --only ruff --json`; `py -3.12 scripts\check_doc_sizes.py --check`; `py -3.12 scripts\sync_badges.py --check`; roadmap/doc pytest gates passed.
