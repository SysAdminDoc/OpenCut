...
- Verification: focused WebView permission-split/schema/docs pytest passed (14 tests), focused Ruff passed for the WebView permission test slice, UXP `node --check` passed, roadmap mirror/lint pytest passed (15 tests), badge sync passed, doc-size checks passed within tolerance, roadmap source lint exited 0 with existing appendix warnings, `rtk git diff --check` passed, `py -3.12 scripts\release_smoke.py --only ruff --json` passed, and `py -3.12 scripts\release_smoke.py --only pytest-fast --json` passed (100 gate tests; 827 pytest cases executed).
