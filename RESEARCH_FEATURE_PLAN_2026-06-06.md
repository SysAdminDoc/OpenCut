...
- **Current behavior:** CEP panel has 1,190 `data-i18n` attributes and a drift-lint test. UXP now has a local locale loader plus first-viewport, Cut-tab, Captions-tab, FCC display-settings, Audio-tab, Video-tab, Timeline-tab, Search-tab, Deliverables-tab, Agent-tab, Settings-tab, generated Settings status, shared runtime-toast coverage, Cut runtime-feedback coverage, Captions runtime-feedback coverage, Audio runtime-feedback coverage, Video core/AI-effects/Shorts runtime-feedback coverage, Timeline/Search/Deliverables/Agent/Settings runtime-feedback coverage, and a full current Spanish locale pack with placeholder parity, but broader non-English locale parity remains open.
...
2. **Only one non-English locale has full current coverage** -- i18n now has `locales/en.json` plus `locales/es.json` covering the current UXP catalogue with placeholder parity; additional translated locale packs remain open
