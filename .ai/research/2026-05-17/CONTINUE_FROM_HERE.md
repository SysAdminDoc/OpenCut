# OpenCut Research — CONTINUE FROM HERE (for Pass 81)
...
**Last update:** 2026-05-18 (after Pass 80; Passes 1-34 all ran on 2026-05-17)
**Session state:** all mandated artefacts exist; Passes 4-75 are summarized in ROADMAP.md and PROJECT_CONTEXT.md. Pass 76 closed F220-F222 (RVC backend execution, natural-language AI auto-color grading, and cut-point pacing/genre-template analysis). Pass 77 closed F224 (AI-facing deepfake detector alias plus evidence/report metadata). Pass 78 closed F228-F230 (review-bundle voice-note attachments and optional HLS browser-scrubbing renditions). Pass 79 closed F232 (Headscale/Tailscale command-plan descriptors for cross-site review portal sharing). Pass 80 closed F235 (WCAG 3 draft descriptive transcript and extended AD timing hooks). This file documents deferred research/product work for a future Pass 81+, not a broken or incomplete research run.
...
1. **Continue the remaining queue.** F121/F122/F127a/F130/F133/F135, F123, F126, F128, F137, F139, F140, F147, F176 (+follow-up), F177, F178, F180, F181, F184, F185, F192, F193, F194, F196, F198, F200, F201, F203, F206, F210, F211, F212, F213, F214, F215, F216, F217, F220, F221, F222, F223, F224, F225, F226, F227, F228, F229, F230, F231, F232, F233, F234, F235, F238, F239, F242, F249, F250, F251, F254, F255, F256, F257, F258, F259, F260, F263, F267, F271, F272 are closed. F205 still needs a runner where coverage can finish. F182 (issue seeder) may still depend on a working `gh` auth context. F252.1 and the F252.2 dispatcher slice are complete but F252 remains open for live WebView/in-Premiere UDT result capture. The next Later-tier candidates are F245-F248/F253. F268 requires an Adobe developer account/storefront process. Larger Pass-1 Now items still requiring model integration: F149/F162/F163/F167/F169.
...
| Item | Status |
|---|---|
| F210 | **DONE** — CEP/UXP panel utility seams now have Vitest coverage and CI/release-smoke enforcement. |
| CEP helpers | `extension/com.opencut.panel/client/panel-utils.js` owns HTML escaping, ExtendScript double-quoted string escaping, lazy DOM proxying, and command-palette section/index scoring for production use and tests. |
| UXP helpers | `extension/com.opencut.uxp/uxp-utils.js` owns UXP HTML escaping and safe DOM-id segment normalization. |
| Guardrails | `npm test` runs 8 Vitest specs; release smoke includes `panel-unit`; Release Full runs `npm test`; PR Fast installs panel dependencies so the release-smoke `panel-unit` step can run. |
...
| Item | Status |
|---|---|
| F232 | **DONE** — review portal share responses now have an optional Headscale/Tailscale operator plan for cross-site review. |
| Headscale boundary | The descriptor validates the self-hosted control-plane URL, strips query/fragment data, rejects embedded credentials, and returns command arrays without running Headscale or Tailscale from Flask. |
| Command plan | Responses include `headscale preauthkeys create` and `tailscale up --login-server ...` arrays with sanitized user, machine name, tags, TTL, host, port, and signed portal URL metadata. |
| Docs/tests | `docs/REVIEW_PORTAL.md` documents the optional cross-site path and `tests/test_review_portal.py` pins core and route behavior. |
| Validation | PASS — `py_compile`, focused review-portal tests (`8 passed`), focused Ruff, and route/MCP generated checks. |
| Files to review | `opencut/core/review_portal.py`, `opencut/routes/platform_infra_routes.py`, `tests/test_review_portal.py`, `docs/REVIEW_PORTAL.md`, and roadmap/state docs. |

### Pass 80 checkpoint

| Item | Status |
|---|---|
| F235 | **DONE** — `/audio/description/microsoft-draft` now has WCAG 3 Working Draft compatibility hooks for descriptive transcripts and extended AD timing. |
| Descriptive transcript | Opt-in draft output can include ordered dialogue plus visual-description events with display text for assistive-technology review. |
| Extended AD timing | When foreground-audio gaps are too short, opt-in output preserves the full AD script, marks cues as `timing_mode="extended"`, and emits machine-readable pause/resume metadata. |
| Docs/tests | `docs/AUDIO_DESCRIPTIONS.md` cites the W3C WCAG 3 Working Draft and WAI media guidance while keeping the non-normative draft boundary explicit. |
| Validation | PASS — `py_compile`, focused audio-description tests (`13 passed`), focused generated-surface tests (`27 passed`), focused Ruff, and route/MCP generated checks. |
| Files to review | `opencut/core/audio_description.py`, `opencut/routes/audio_advanced_routes.py`, `tests/test_audio_advanced.py`, `docs/AUDIO_DESCRIPTIONS.md`, generated route manifest, and roadmap/state docs. |

### Pass 75 checkpoint

| Item | Status |
|---|---|
| F212 | **DONE** — WPF installer now has xUnit coverage plus Windows-runner quiet install/uninstall smoke wiring. |
| Quiet mode | `InstallerCommandLineOptions` parses `--install/--uninstall --quiet`, temp profile/log flags, and safety toggles for shortcuts, CEP, PATH updates, HKLM registration, Whisper downloads, and optional dependencies. |
| Installer safety | `InstallConfig` now supports configurable `UserDataPath`; uninstall honors the CEP/shortcut/PATH/registration toggles so CI smoke does not touch real developer state. |
| Test suite | `installer/tests/OpenCut.Installer.Tests/` covers command-line parsing, path derivation, installer manifest path override, file copy behavior, progress math, and `SubStream` windowed reads. |
| CI wiring | Release Full runs `scripts/test_wpf_installer.ps1` on Windows and runs `scripts/smoke_wpf_installer.ps1` after the WPF artifact is built. `tests/test_wpf_installer_test_suite.py` is part of release smoke. |
| Build fix | `OpenCut.Installer.csproj` now points `ApplicationIcon` at tracked `Assets/logo.ico`, fixing source builds. |
| Validation | PASS — `scripts/test_wpf_installer.ps1` (`13 passed`), focused installer Python guards (`15 passed`), and `dotnet build installer\src\OpenCut.Installer\OpenCut.Installer.csproj -c Release --nologo` passed locally. The destructive headless smoke remains CI-guarded for Windows release runners with a built self-extracting artifact. |
| Files to review | `installer/src/OpenCut.Installer/CommandLine/`, `installer/src/OpenCut.Installer/Models/InstallConfig.cs`, `installer/src/OpenCut.Installer/Services/{InstallEngine,UninstallEngine}.cs`, `installer/tests/OpenCut.Installer.Tests/`, `scripts/test_wpf_installer.ps1`, `scripts/smoke_wpf_installer.ps1`, `.github/workflows/build.yml`, `tests/test_wpf_installer_test_suite.py`, and roadmap/state docs. |

### Pass 73 checkpoint

| Item | Status |
|---|---|
| F206 | **DONE** — pull requests now use a Linux-only PR Fast workflow instead of the three-OS Release Full matrix. |
| PR Fast | `.github/workflows/pr-fast.yml` installs Python 3.12, FFmpeg, `ruff`, `pytest`, `opencut[standard]`, and runs `scripts/release_smoke.py --json` with release-only heavy/drift checks skipped. |
| Release Full | `.github/workflows/build.yml` is renamed Release Full, no longer has a `pull_request` trigger, and still runs on pushes to `main`, tags, and manual dispatch. |
| Guardrails | `tests/test_ci_workflow_split.py` pins the split and is registered in `pytest-fast`. |
| Validation | PASS — focused F206 tests (`4 passed`), `py_compile`, and focused Ruff passed locally. |
| Files to review | `.github/workflows/pr-fast.yml`, `.github/workflows/build.yml`, `tests/test_ci_workflow_split.py`, `scripts/release_smoke.py`, `ROADMAP.md`, `CHANGELOG.md`, `PROJECT_CONTEXT.md`, and this file. |
