# OpenCut Research — CONTINUE FROM HERE (for Pass 56)
...
- **Live counts:** 1,370 routes / 101 blueprints / 528 core modules / 147 test files / 47 model cards / 117 public `check_*` probes (86 `check_*_available`) / 84 `FeatureRecord` entries / 39 curated MCP tools / 1,312 opt-in extended MCP tools / 100 OpenAPI-typed endpoints / 18 CEP host functions (2 CEP-only).
...
1. **Continue the remaining queue.** F121/F122/F127a/F130/F133/F135, F123, F126, F128, F137, F139, F140, F147, F176 (+follow-up), F177, F178, F180, F181, F184, F185, F192, F194, F198, F200, F201, F203, F211, F213, F214, F215, F216, F217, F223, F225, F226, F227, F229, F231, F233, F234, F238, F242, F251, F259 are closed. F205 still needs a runner where coverage can finish. F182 (issue seeder) may still depend on a working `gh` auth context. The next small standards/packaging candidates are F239/F249/F250 depending on whether fresh standards or packaging research is required. F193 remains the larger schema-introspection follow-up to F192/F194. Larger Pass-1 Now items still requiring model integration: F149/F162/F163/F167/F169.
...
| Generated catalogue | `opencut/_generated/mcp_extended_tools.json` contains 1,312 `opencut_route_*` tools generated from `opencut/_generated/route_manifest.json`, `opencut/_generated/api_aliases.json`, and the OpenAPI response-schema map. |
...
1. **Run `python -m opencut.tools.dump_route_manifest --check`** to verify the cached 1,370-route figure against live `url_map`. Possible drift after future route commits.
...
`/system/feature-state` now exposes 84 feature records. The generated F191 manifest covers direct route functions that visibly call public `checks.py` probes. It is not a full per-route readiness matrix for all 1,370 routes; deeper core-only gates still belong to F196/F209.
