# OpenCut Research — CONTINUE FROM HERE (for Pass 53)
...
- **Live counts:** 1,367 routes / 101 blueprints / 526 core modules / 144 test files / 47 model cards / 117 public `check_*` probes (86 `check_*_available`) / 84 `FeatureRecord` entries / 39 curated MCP tools / 1,309 opt-in extended MCP tools / 100 OpenAPI-typed endpoints / 18 CEP host functions (2 CEP-only).
...
| Generated catalogue | `opencut/_generated/mcp_extended_tools.json` contains 1,309 `opencut_route_*` tools generated from `opencut/_generated/route_manifest.json`, `opencut/_generated/api_aliases.json`, and the OpenAPI response-schema map. |
...
1. **Run `python -m opencut.tools.dump_route_manifest --check`** to verify the cached 1,367-route figure against live `url_map`. Possible drift after future route commits.
...
`/system/feature-state` now exposes 84 feature records. The generated F191 manifest covers direct route functions that visibly call public `checks.py` probes. It is not a full per-route readiness matrix for all 1,367 routes; deeper core-only gates still belong to F196/F209.
