...
`tests/test_route_manifest.py` pins the manifest contract. There's no test asserting the OpenAPI spec generated by `opencut/openapi.py` is well-formed (against the OpenAPI 3.0.3 JSON Schema) or that every endpoint has at least a 200/400/403 response. **F208 — OpenAPI spec validity test** (small).
...
|---|---|---|---|
| F208 | OpenAPI spec validity test (per-endpoint 200/400/403) | Now | S |
