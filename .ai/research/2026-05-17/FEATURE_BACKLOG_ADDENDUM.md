...
|---|---|---|---|---|---|
| F191 | Auto-derive `FeatureRecord` from check functions + route manifest | Replace 29-record hand-maintained registry with auto-derived catalogue covering all ~300 dep-gated routes | ROUTE_READINESS_AUDIT §3 | M | Now |
...
| F197 | `NON_AI_CHECKS` allowlist in `registry.py` | Mirror of F115 allowlist | ROUTE_READINESS_AUDIT §6 | S | Now |
...
| F199 | Document `/api/*` alias policy + generate alias map | 233 `/api/*` routes mirror canonical paths | ROUTE_READINESS_AUDIT §8 | S | Now |
...
| F202 | Apple notarisation for macOS PyInstaller bundle | **Mandatory for Homebrew Cask Sept 1, 2026** per niche AI subagent §5 | INSTALLER_AUDIT §7 + niche AI §5 | M | **Now** (priority bump from "Next" due to Sept 1 deadline) |
...
| F213 | Inno Setup install/uninstall smoke in CI | Verify shortcuts + cleanup | TEST_COVERAGE §3.6 | M | Next |
...
| F240 | Per-target reading-speed profile (Netflix 17 cps / BBC 160-180 wpm / YouTube 220 wpm / FCC 180-200 wpm) | Caption QC extension | Niche AI §2 | S | Now |
| F241 | HarfBuzz-mandatory CI gate (libass + Pillow + Skia) | RTL / CJK / Indic rendering correctness | Niche AI §3 + F223 | S | Now |
...
|---|---|---|
| **Now** | F191, F195, F197, F199, F202 (priority-bumped), F204, F205, F207, F208, F209, F218, F219, F236, F237, F240, F241, F243, F244, F251, F259 | 20 |
